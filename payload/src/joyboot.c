...
/* Original address: 0x0201B0A4 */
extern char sJoybootGbaHandshake[];

/* Original address: 0x0201B0AC */
extern char sJoybootGameCubeHandshake[];

/* Original address: 0x0201907C */
s32 Swap32(u32 *value) {
    u8 bytes[4];
    u8 *value_bytes = (u8 *)value;

    bytes[0] = value_bytes[3];
    bytes[1] = value_bytes[2];
    bytes[2] = value_bytes[1];
    bytes[3] = value_bytes[0];

    return *(s32 *)bytes;
}

/* Original address: 0x02019098 */
